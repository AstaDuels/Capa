--[[
    ASTA REMAKE â€“ Full Hub (Simplificado)
    Speed Booster (LinearVelocity) integrado ao Carry Speed e Lagger.
    BotÃµes mÃ³veis individuais + fundo personalizado.
    BotÃ£o de Lock na barra superior.
...
-- ============================================================
-- ANTI-KICK GATE (global, usado por funcoes definidas antes da API)
-- Enquanto o gate estiver "blocked", NENHUMA velocidade extra
-- (booster, carry, lagger, auto-path, override de Velocity)
-- pode agir -> nao toma kick nos 5s de cooldown / no PvP.
-- ============================================================
_G.ASTA_BLOCKED = _G.ASTA_BLOCKED or function() return false end
local function AK_BLOCKED()
    local f = _G.ASTA_BLOCKED
    return (type(f) == "function") and f() == true
end

-- ============================================================
-- SPEED BOOSTER (LinearVelocity) â€“ CORRIGIDO
-- ============================================================
...
-- FunÃ§Ã£o auxiliar para obter a velocidade efetiva do booster
local function getEffectiveBoosterSpeed()
    ...
    elseif carrySpeedActive then
        ...
        return NS  -- fallback (nÃ£o deve ser usado)
    end
end
...
-- FunÃ§Ã£o centralizada para atualizar o booster
local function updateBooster()
    local shouldActivate = (carrySpeedActive or laggerModeEnabled) and not AK_BLOCKED()
...
-- ============================================================
-- ANTI-RAGDOLL SIMPLES (substitui a versÃ£o antiga)
-- ============================================================
...
-- FunÃ§Ã£o auxiliar para ser usada nos toggles
...
-- ============================================================
-- NOVA FUNÃ‡ÃƒO DROP (com ApplyImpulse)
-- ============================================================
...
function runDrop()
    if not dropEnabled then
        print("Drop estÃ¡ desativado.")
        return
    end
    if dropActive then
        print("Drop jÃ¡ em execuÃ§Ã£o.")
        return
    end
...
-- ============================================================
-- SISTEMA DE RESET (substitui Insta Reset e Medusa Reset) â€“ MAIS RÃPIDO
-- ============================================================
...
local ASTA={antiKick=true}  -- ANTI-KICK LIGADO POR PADRAO
...
-- ============================================================
-- ANTI-KICK API (janela de seguranca de 5s antes/durante o PvP)
-- ============================================================
local AK_WINDOW = 5.0          -- segundos de bloqueio apos pegar/soltar o pet
local akWindowUntil = 0
local akLastBrainrot = false

function ASTA.armAntiKickWindow(secs)
    local t = os.clock() + (secs or AK_WINDOW)
    if t > akWindowUntil then akWindowUntil = t end
end

function ASTA.holdingBrainrot()
    return ASTA.antiKick and brainrotDetected
end

function ASTA.blocked()
    if not ASTA.antiKick then return false end
    if brainrotDetected then return true end
    return os.clock() < akWindowUntil
end

function ASTA.antiKickRemaining()
    if brainrotDetected then return AK_WINDOW end
    return math.max(0, akWindowUntil - os.clock())
end

-- gate global consumido pelo booster/loops definidos acima
_G.ASTA_BLOCKED = function() return ASTA.blocked() end

function ASTA.tryStart(label)
    if ASTA.blocked() then
        warn(("Anti-Kick: %s bloqueado (pet equipado / janela de %ds)"):format(label or "acao de velocidade", AK_WINDOW))
        return false
    end
    return true
end
...
-- ============================================================
-- BAT V2 (Aimbot + perseguiÃ§Ã£o automÃ¡tica)
-- ============================================================
...
-- ============================================================
-- CYBER EXTRAS (background + animaÃ§Ãµes)
-- ============================================================
...
-- ============================================================
-- MOBILE BUTTONS â€“ INDIVIDUAIS ARRASTÃVEIS (sem aimbot)
-- ============================================================
    ...
    local function makeButton(def, index)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "autoLeft" then
                if not autoLeftEnabled and not ASTA.tryStart("Auto Left") then setOn(false); return end
            ...
            if def.key == "autoRight" then
                if not autoRightEnabled and not ASTA.tryStart("Auto Right") then setOn(false); return end
...
-- ============================================================
-- FUNÃ‡Ã•ES EXISTENTES (anti-ragdoll, steal, etc)
-- ============================================================
...
local function getActiveMoveSpeed()
    if ASTA.blocked() then return NS end
...
local function updateAutoSwitchSpeed()
    ...
    if ASTA.blocked() then return end
    local char=LP.Character;if not char then return end
...
local function executeSteal(prompt)
    ...
    data.ready=false;isStealing=true;stealStartTime=tick();ASTA.armAntiKickWindow(AK_WINDOW)
    task.spawn(function()
...
RunService.RenderStepped:Connect(function()
    ...
    if ASTA.blocked() then
        -- ANTI-KICK: movimento 100% vanilla durante os 5s / com pet na mao
...
startAutoLeft=function()
    if not ASTA.tryStart("Auto Left") then autoLeftEnabled=false; if autoLeftSetVisual then autoLeftSetVisual(false) end; if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end; return end
...
startAutoRight=function()
    if not ASTA.tryStart("Auto Right") then autoRightEnabled=false; if autoRightSetVisual then autoRightSetVisual(false) end; if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end; return end
...
-- ============================================================
-- ANTI KICK (Asta) â€“ VERSÃƒO ROBUSTA 100%
-- ============================================================
function ASTA.enableAntiKick()
    ASTA.antiKick=true
    task.spawn(function()
        while ASTA.antiKick do
            task.wait(0.1) -- Verifica mais rÃ¡pido (0.1s) para reagir instantaneamente
            local char=LP.Character
            if char then
                ...
                if hasBrainrot ~= akLastBrainrot then
                    -- pegou OU soltou o pet: arma a janela de 5s
                    ASTA.armAntiKickWindow(AK_WINDOW)
                    akLastBrainrot = hasBrainrot
                end
                brainrotDetected = hasBrainrot

                if hasBrainrot or ASTA.blocked() then
                    -- ===== FORÃ‡A DESATIVAR TODA E QUALQUER VELOCIDADE EXTRA =====
                    ...
                    -- Desativa paths automÃ¡ticos tambÃ©m
                    if autoLeftEnabled then autoLeftEnabled=false;stopAutoLeft();if autoLeftSetVisual then autoLeftSetVisual(false) end;if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end end
                    if autoRightEnabled then autoRightEnabled=false;stopAutoRight();if autoRightSetVisual then autoRightSetVisual(false) end;if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end end
                end
            end
...
-- ============================================================
-- SAVE CONFIG (atualizado com Bat V2)
-- ============================================================
saveConfig = function()
    local function ks(e)
...
toggleCarryMode=function()
    if not carrySpeedActive and not ASTA.tryStart("Carry Speed") then
        print(("âš ï¸ Anti-Kick: Carry Speed bloqueado (%.1fs restantes)"):format(ASTA.antiKickRemaining()))
        return
    end
...
toggleLaggerMode=function()
    if not laggerModeEnabled and not ASTA.tryStart("Lagger Mode") then
        print(("âš ï¸ Anti-Kick: Lagger Mode bloqueado (%.1fs restantes)"):format(ASTA.antiKickRemaining()))
        return
    end
...
local function speedToggleAction() 
    if not ASTA.tryStart("Tecla de velocidade") then return end
    toggleCarryMode() 
    saveConfig() 
end
...
-- ============================================================
-- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled, batV2Enabled)
-- ============================================================
pcall(function()
...
;(function()
    ...
    ML.Text="ASTA REMAKE â€¢ PREMIUM"; ML.TextColor3=C.textDim; ML.TextSize=10; ML.Font=Enum.Font.GothamBold
    ML.TextXAlignment=Enum.TextXAlignment.Left; ML.Parent=HF; ML.ZIndex=3

    -- BOTÃƒO DE UNLOCK/LOCK
...
ASTA.antiKick=false; ASTA.enableAntiKick() -- ANTI-KICK sempre inicia ligado
...
print("âœ… ASTA REMAKE LOADED â€“ Anti-Kick 100% robusto + Bat V2 (tecla B)")
